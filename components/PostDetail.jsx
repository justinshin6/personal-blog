...
const PostDetail = ({post}) => {
    ...
    // function that modifies text fragments and returns the modified text
    const getContentFragment = (index, text, obj, type) => {
        ...
        if (obj) {
            ...
            // if italic, then modify
            if (obj.italic) {
            modifiedText = (<em key={index}>{text}</em>)
            }
            // if underline, then modify
            if (obj.underline) {
    ...
    return (
        <div className='bg-gray-700 bg-opacity-30 shadow-lg rounded-lg lg:p-8 pb-12 mb-8'>
            <div className="relative overflow-hidden shadow-md mb-6">
                <img 
                    src={post.featuredImage.url}
                    alt={post.title}
                    className='object-top h-full w-full rounded-t-lgs'
                />
            </div>
            <div className=" px-4 lg:px-0">
                ...
                <h1 className='mb-8 text-3xl text-white font-semibold'>
                    {post.title}
                </h1>

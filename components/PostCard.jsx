...
const PostCard = ({post}) => {
    return (
        <div className="bg-gray-700 bg-opacity-30 shadow-lg rounded-lg p-0 lg:p-8 pb-12 b-8 mb-8">
            <div className="relative overflow-hidden shadow-md pb-80 mb-6">
                <img src={post.featuredImage.url}
                alt={post.title}
                className='object-top absolute h-80 w-full object-cover shadow-lg rounded-t-lg lg:rounded'/>
            </div>
            <h1 className="  text-center mb-8 cursor cursor-pointer
             text-3xl font-semibold">
                <Link href={`/post/${post.slug}`}>
                    <span className="transition duration-700 text-white hover:text-sky-400">
                    {post.title}
                    </span>
                </Link>
            </h1>
            ...
            <div className="text-center">
                <Link href={`/post/${post.slug}`}>
                    <span className='transition duration-500 transform hover:-translate-y-1 inline-block bg-sky-400
